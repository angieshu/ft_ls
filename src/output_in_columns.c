...
void	col_row(t_col *w_info, t_list *list)
{
	...
	w_info->row = 1;
	w_info->col = (w_info->col_max / 2) + 1;
	while (w_info->col <= w_info->col_max)
	{
		if (w_info->col * w_info->row >= w_info->n_files)
			break ;
		if (w_info->col == w_info->col_max)
		{
			w_info->col = (w_info->col_max / 2) + 1;
			w_info->row++;
		}
...
void	columns(t_list *list, int i)
{
	...
	col_row(&w_info, list);
	while (get_elem_n(list, i) &&
		i <= w_info.row && w_info.n_files > w_info.col_max)
	{
		n = i;
